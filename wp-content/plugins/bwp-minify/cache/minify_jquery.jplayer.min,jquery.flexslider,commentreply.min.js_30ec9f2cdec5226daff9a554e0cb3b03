...
else if($(this).data('flexslider')!=true){new $.flexslider($(this),options);}});}})(jQuery);jQuery(document).ready(function(){jQuery.noConflict();FLEX.pauseOnHover=(FLEX.pauseOnHover=='enable')?true:false;FLEX.pauseOnAction=(FLEX.pauseOnAction=='enable')?true:false;FLEX.controlNav=(FLEX.controlNav=='enable')?true:false;FLEX.directionNav=(FLEX.directionNav=='enable')?true:false;jQuery('.flexslider').not('.flexslider-gallery').flexslider(FLEX);jQuery('.flexslider-gallery').each(function(){FLEX.slideshowSpeed=(Math.random()*3000)+3000;FLEX.animationDuration=1500;jQuery(this).flexslider(FLEX);});});

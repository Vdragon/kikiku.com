...
if ( ! function_exists( 'mv_dynamic_to_top_field_text' ) ) {    
    function mv_dynamic_to_top_field_text() {
        echo mv_dynamic_to_top_do_textfield( 'text', '邻居家的小猫咪说了:<br />不留脚印会被鄙视滴!<br /><span style=\'color:#2F4F4F;font-weight:bold;\'>♂ </span><span style=\'color:#ffffff;font-weight:bold;\'>我要发评论</span><span style=\'color:#2F4F4F;font-weight:bold;\'> ♀</span>', __( 'Text displayed on the button.', 'dynamic-to-top' ), 'regular-text' );
    }
}
...
if ( ! function_exists( 'mv_dynamic_to_top_do_slider' ) ) { 
    function mv_dynamic_to_top_do_slider( $meta, $value = "", $desc = "", $class = "" )
    {
       ...
       return '<div id="slider-picker-'.$sanitized_meta_title.'" class="dtt-slider"></div><input type="text" name="dynamic_to_top['.$meta.']" id="slider-'.$sanitized_meta_title.'" '.$class.' value="'.$value.'" size="1800"/>'.$desc;    
    }
}